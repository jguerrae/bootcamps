...
const promise = new Promise(resolve => {
  fs.readdir(envPath, function (err, files) {
    //handling error
    if (err) {
...
const defaultFacets = ["collections_names", "collections_slugs", "productVariants.stock_quantity", "productVariants.availableForSale", "kInventory"];

const customFacet = {
  kInventory: "kInventory"
};
const defaultsSearchableAttributes = ["name", "collections_names", "collections_slugs", "productVariants.description", "productVariants.stock_quantity"];
const enviroment = process.env.NODE_ENV || "development";

const client = enviroment == "development" ? algoliasearch("DL3F82QV71", "f2bf466c1d2c6164cbb06a51b377c288") : enviroment == "production" ? algoliasearch("DL3F82QV71", "f2bf466c1d2c6164cbb06a51b377c288") : null;
const internalIndexes = {};

const createReplicaIndex = (eCommerceId, eComerceDomain, eComerceCurrencies, locale, replicas) => {
  replicas.map(e => {
    const repInde = client.initIndex(e.full);
    internalIndexes[`${eCommerceId}_${eComerceDomain}_${locale.code}_` + e.param] = repInde;
...
const setUpIndex = (eCommerceId, eComerceDomain, eComerceCurrencies, algoliaConfig, locale, resolve, reject) => {
  // se crea el index dependi del env, y se agrega a los internos
  const indexName = `${eCommerceId}_${eComerceDomain}_${locale.code}${enviroment == "development" ? "_dev" : ""}`;
  const indexTemp = client.initIndex(indexName);
  internalIndexes[`${eCommerceId}_${eComerceDomain}_${locale.code}`] = indexTemp;
  ...
  indexTemp.setSettings(Object.assign({}, algoliaConfig, {
    ...
    createReplicaIndex(eCommerceId, eComerceDomain, eComerceCurrencies, locale, fullRepNames);
...
const createIndex = (eco, full = true) => {
  ...
  if (algolia_index && algolia_index == true && (isDev ? isKlaus : true)) {
    ...
    const fullSearchable = defaultsSearchableAttributes.concat(searchableAttributes);
    ...
    const localesMap = eco.locales.map(async loc => {
      ...
      if (full) {
        return new Promise((resolve, reject) => {
          setUpIndex(eco._id, eco.domain, eco.currencies, algoliaConfig, loc, resolve, reject);
        });
      } else {
